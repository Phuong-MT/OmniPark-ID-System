...
const mockParks = [
    { id: 1, name: 'Alpha Gateway', location: 'North District', status: 'active', occupancy: '85%', devices: 12 },
    { id: 2, name: 'Beta Sector', location: 'South District', status: 'maintenance', occupancy: '42%', devices: 8 },
    { id: 3, name: 'Gamma Hub', location: 'East District', status: 'active', occupancy: '91%', devices: 15 },
    { id: 4, name: 'Delta Point', location: 'West District', status: 'offline', occupancy: '---', devices: 5 },
];
            ...
            <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
                <div>
                    ...
                    <p className="text-zinc-500 dark:text-zinc-400">
                        {role === 'POC' ? 'Manage your assigned parking facilities.' : 'Manage all parking facilities across the network.'}
                    </p>
                </div>
            ...
            <div className="grid gap-6 md:grid-cols-3">
                <div className="md:col-span-2 grid gap-4 grid-cols-1 sm:grid-cols-2">
                    {displayParks.map((park) => (
                        <Card key={park.id} className="hover:border-blue-500 dark:hover:border-blue-500 cursor-pointer transition-colors">
                            <CardHeader className="pb-2">
                                <div className="flex items-center justify-between">
                                    <CardTitle className="text-lg">{park.name}</CardTitle>
                                    <Badge variant={park.status === 'active' ? 'success' : park.status === 'maintenance' ? 'warning' : 'destructive'}>
                                        {park.status}
                                    </Badge>
                                </div>
                        ...
                        <CardContent className="flex-1 flex flex-col">
                            <div className="flex-1 rounded-md bg-zinc-100 dark:bg-zinc-800/50 flex items-center justify-center border border-zinc-200 dark:border-zinc-800 relative overflow-hidden">
                                ...
                                <div className="flex flex-col items-center gap-2 relative z-10 text-zinc-500 dark:text-zinc-400">
                                    ...
                                    <span className="text-sm font-medium">Select a park to view</span>
                                </div>
                            </div>
                        </CardContent>
                    </Card>
