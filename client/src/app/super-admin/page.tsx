    ...
    if (role !== 'SUPER_ADMIN') {
        return (
            <div className="flex flex-col items-center justify-center p-12 text-center h-[50vh]">
                ...
                <p className="text-zinc-500 max-w-sm">This area is strictly for Super Administrators. System configuration changes are prohibited for your role.</p>
            </div>
        );
    }
            ...
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
                ...
                <Card className="bg-zinc-950 text-zinc-50 dark:bg-zinc-50 dark:text-zinc-950">
                    ...
                    <CardContent className="h-full">
                        <p className="text-xs opacity-70 mb-2 font-mono">sys&gt; systemctl restart core</p>
                        <Badge variant="secondary" className="bg-zinc-800 text-zinc-100 dark:bg-zinc-200 dark:text-zinc-900 cursor-pointer">Execute</Badge>
                    </CardContent>
                </Card>
            </div>
            ...
            <div className="grid gap-6 md:grid-cols-2">
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription>Enable or disable beta features across all tenants.</CardDescription>
                    </CardHeader>
                    <CardContent className="space-y-4">
                        {[
                            { id: 'f_lpr_v2', name: 'LPR Engine v2', status: true, desc: 'Uses new ML model for license plate recognition' },
                            { id: 'f_payment_stripe', name: 'Stripe Integration', status: true, desc: 'Allow tenants to use Stripe directly' },
                            { id: 'f_mqtt_bridge', name: 'Legacy MQTT Bridge', status: false, desc: 'Support for v1.x hardware' },
                        ].map(feature => (
                            <div key={feature.id} className="flex items-center justify-between py-3 border-b border-zinc-100 dark:border-zinc-800 last:border-0 last:pb-0">
                                <div className="space-y-1">
                                    <div className="flex items-center gap-2">
                                        <p className="text-sm font-medium leading-none">{feature.name}</p>
                                        <Badge variant="outline" className="text-[10px] h-4 py-0 px-1">{feature.id}</Badge>
                                    </div>
                                    <p className="text-xs text-zinc-500">{feature.desc}</p>
                                </div>
                                <div className={`w-10 h-6 rounded-full relative cursor-pointer border-2 border-transparent transition-colors ${feature.status ? 'bg-purple-600' : 'bg-zinc-200 dark:bg-zinc-800'}`}>
                                    <div className={`w-5 h-5 bg-white rounded-full absolute top-0 shadow-sm ${feature.status ? 'right-0' : 'left-0'}`} />
                                </div>
                            </div>
                        ))}
                    </CardContent>
