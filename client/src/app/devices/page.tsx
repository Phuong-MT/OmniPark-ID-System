						...
						<table className="w-full caption-bottom text-sm">
							<thead className="[&_tr]:border-b border-zinc-200 dark:border-zinc-800">
								<tr className="border-b transition-colors hover:bg-zinc-100/50 data-[state=selected]:bg-zinc-100 dark:hover:bg-zinc-800/50 dark:data-[state=selected]:bg-zinc-800">
									...
									<th className="h-12 px-4 text-left align-middle font-medium text-zinc-500 text-right">
										Actions
									</th>
								</tr>
							</thead>
								...
								{mockDevices.map((device) => (
									...
									>
										...
										<td className="p-4 align-middle">
											<div className="flex items-center gap-2">
												<span
													className={`flex h-2 w-2 rounded-full ${device.status === "online" ? "bg-green-500" : device.status === "pairing" ? "bg-yellow-500 animate-pulse" : "bg-red-500"}`}
												/>
										...
										<td className="p-4 align-middle">
											<Badge
												variant={device.paired ? "secondary" : "warning"}
											>
