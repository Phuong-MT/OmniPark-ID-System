...
describe('DevicesService', () => {
  ...
  describe('createDevice', () => {
    it('should create a new device', async () => {
      ...
      const mockDevice = { macAddress: 'AA:BB:CC', status: DeviceStatus.ACTIVE };
  ...
  describe('findDevices', () => {
    it('should return devices list', async () => {
      ...
      const result = await service.findDevices({ tenantCode: 'tenant1', status: DeviceStatus.ACTIVE });

      expect(deviceModel.find).toHaveBeenCalledWith({ tenantCode: 'tenant1', status: DeviceStatus.ACTIVE });
  ...
  describe('findByDeviceId', () => {
    it('should find device by deviceId', async () => {
      ...
      expect(deviceModel.findOne).toHaveBeenCalledWith({ deviceId: 'dev1' });
      expect(result).toEqual(mockResult);
    });
    ...
    it('should throw NotFoundException if not found', async () => {
      ...
      await expect(service.findByDeviceId('dev1')).rejects.toThrow(NotFoundException);
    });
  });

  describe('activateDevice', () => {
    it('should activate a paired device', async () => {
      const mockDevice = { status: DeviceStatus.INACTIVE, save: jest.fn() };
    ...
    it('should throw NotFoundException if invalid token', async () => {
      ...
      await expect(service.activateDevice('AA:BB:CC', 'invalid_token')).rejects.toThrow(NotFoundException);
    });
  });
});
